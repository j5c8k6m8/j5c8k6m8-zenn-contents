...
デッドコードレシピは、実際のコードで示すため細かく定義する。そのため、デッドコードレシピで似ているものが存在する。似ているデッドコードレシピを **`🔖デッドコードパターン🔖`** として、本書ではパターンごとにデッドコードレシピを紹介する。また、デッドコードを作るための部品を必要に応じて、 **`🔪デッドコードツール🔪`** として紹介する。
...
::: message
アドベントカレンダー期間に公開していくことを考え、まずは、`Python`, `Ruby`, `JavaScript`, `Java`, `Go` に限定する。将来的には、`TypeScript`, `Rust`, `Scala` あたりから対象言語を増やしていく予定。
:::

デッドコードへの対応は、プログラム言語の機能のみではなく、静的解析ツールを使う場合も多い。本書では、 **著名な静的解析ツールについては対象** とする。

対象のプログラミング言語のバージョンや静的解析ツールなどの環境については 「**付録A. 対象言語と環境**」 に記載する。
...
**[本書のGitHub](https://github.com/j5c8k6m8/j5c8k6m8-zenn-contents/tree/zenn/books)のPR(プルリクエスト)** 、 **Zennの本書の紹介記事のディスカッション** 、 **twitterのハッシュタグ([#デッドコードクックブック](https://twitter.com/hashtag/%E3%83%87%E3%83%83%E3%83%89%E3%82%B3%E3%83%BC%E3%83%89%E3%82%AF%E3%83%83%E3%82%AF%E3%83%96%E3%83%83%E3%82%AF), [#デッドコードレシピ](https://twitter.com/hashtag/%E3%83%87%E3%83%83%E3%83%89%E3%82%B3%E3%83%BC%E3%83%89%E3%83%AC%E3%82%B7%E3%83%94))** などでの連絡をお待ちしています。
...
12月1日時点では半分も執筆できていないため、完成できるかドキドキしています。読者の反応が励みになります。

2020年12月25日以降も更新していくつもりではありますが、2020年のアドベントカレンダーの期間中に色んな意見をもらえると嬉しく感じます！
