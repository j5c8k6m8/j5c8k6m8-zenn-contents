...
本書は特定のプログラム言語に特化せずにパターンを紹介します。パターンによっては、 **プログラム言語毎に、エラーになる、警告になる、記法をサポートしてない、静的解析ツールで検出可能など、状況が異なります**。

本書は、実際のデッドコード除去時の参考となることも目的としているため、 **なるべく多くのサンプルコードを掲載したい** と思っています。しかしながら、筆者の経験しているプログラミング言語は数個しかありません。また、高度なプログラミングに対する知識も十分ではありません。

以下のような場合は、連絡をいただけると非常に助かります。
...
1. GitHubのPR(プルリクエスト)
2. Zennの本書の紹介記事のディスカッション
3. twitterの以下のハッシュタグ
  - #デッドコードクックブック
  - #デッドコードレシピ

本書は筆者だけでは、目的を達成する本とすることができないと考えています。 **いろいろな意見を伺いたい目的で、後述するアドベントカレンダー形式での公開にする** ことにしました。

気になる点などがあれば、連絡いただけると助かります。~~やさしい🪓マサカリ🪓をお待ちしています。~~


## アドベントカレンダー形式での公開について

本書は、2020年12月１日から、12月25日まで、毎日チャプターを一つずつ公開していく方式をとっています。

完成までの道のりが長いと感じたので、筆者自信も期間を切りモチベーションを保ちつつ、執筆しながら意見がもらいやすいのではないか？と考えたためです。

実は、12月1日時点では半分も記載できてなく、完成できるかドキドキしています。読者の反応が励みになります。

2020年12月25日以降も更新していくつもりではありますが、是非ともアドベントカレンダーの期間中に色んな意見をもらえると嬉しく感じます！
