...
また、一部のプログラミング言語では、`コールバック関数` 中で `break` を利用できるケースもある。これは、 `イテレータ` を途中で中断する時などに有効である。`コールバック関数中のbreak` 後のコードも本レシピに含める。なお、 `break` により中断する `コールバック関数後のコード` によるレシピは、 [`🧪breakするコールバック関数呼出後のコード`](./r_after_yield_break) に含まれる。
